...
// "מה חדש בספרייה" — driven straight from the library source of truth.
// New articles are appended to LIBRARY_ARTICLES, so the last items are the newest.
// This section updates automatically whenever an article is added — no manual edit here.
const LATEST_ARTICLES = LIBRARY_ARTICLES.slice(-3).reverse();
...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* No "reveal" class here on purpose: this block's whole job is to be noticed,
          so it must never start at opacity:0 waiting on the IntersectionObserver. */}
      <section className="sec-std" style={{ background:"var(--linen)" }}>
        <div className="container-md" style={{ textAlign:"center" }}>
          <span className="tag-terra" style={{ marginBottom:"16px", display:"inline-flex", alignItems:"center", gap:"6px" }}>
            <SiteIcon name="sparkle" size={14} /> חדש בספרייה
          </span>
          <h2 style={{ fontSize:"clamp(26px,3.6vw,40px)", color:"var(--charcoal)", marginBottom:"16px", lineHeight:1.2 }}>
            מאמרים חדשים נוספו לספרייה
          </h2>
          <div className="divider-accent-center" style={{ marginBottom:"20px" }} />
          <p style={{ fontSize:"16px", color:"var(--charcoal-soft)", lineHeight:1.8, fontFamily:"var(--font-hebrew)", maxWidth:"560px", margin:"0 auto 40px" }}>
            תכנים חדשים לקריאה — כלים מעשיים, רקע מקצועי ושאלות להתבוננות, בשפה פשוטה ובלי אשמה.
          </p>

          <div
            className="library-grid"
            style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))", gap:"22px", textAlign:"right" }}
          >
            {LATEST_ARTICLES.map((article, i) => (
              <Link
                ...
                className="card-lg"
                style={{ padding:"28px 26px", textDecoration:"none", display:"flex", flexDirection:"column", position:"relative" }}
              >
                {i === 0 && (
                  <span
                    className="tag-terra"
                    style={{ position:"absolute", top:"18px", left:"18px", fontSize:"11px", padding:"3px 10px" }}
                  >
                    חדש
                  </span>
                )}
                <span
                  aria-hidden="true"
                  style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:"46px", height:"46px", borderRadius:"var(--radius-pill)", background:"var(--sage-faint)", color:"var(--sage-dark)", marginBottom:"18px" }}
                >
                  <SiteIcon name={article.icon ?? "clock"} size={22} />
                </span>
                <h3 style={{ fontFamily:"var(--font-serif)", fontSize:"20px", fontWeight:600, color:"var(--charcoal)", marginBottom:"12px", lineHeight:1.35 }}>
                  {article.title}
                </h3>
                <p style={{ fontSize:"14.5px", color:"var(--charcoal-soft)", lineHeight:1.75, fontFamily:"var(--font-hebrew)", marginBottom:"20px", flexGrow:1 }}>
                  {article.kicker}
                </p>
                <span style={{ color:"var(--sage-dark)", fontWeight:700, fontSize:"14px", fontFamily:"var(--font-hebrew)" }}>
                  למאמר המלא ←
                </span>
              </Link>
            ))}
          ...
          <div style={{ marginTop:"40px" }}>
            <Link href="/library" style={{ display:"inline-block", color:"var(--terra-dark)", fontWeight:700, textDecoration:"none", fontSize:"15px", fontFamily:"var(--font-hebrew)", borderBottom:"2px solid var(--terra)", paddingBottom:"2px" }}>
              לכל המאמרים בספרייה ←
            </Link>
          </div>
        </div>
      ...
      <section className="sec-cta reveal-scale" style={{ textAlign:"center", position:"relative", overflow:"hidden", background:"linear-gradient(135deg, var(--charcoal) 0%, #3D3530 100%)" }}>
        ...
        <div style={{ position:"relative", zIndex:1, maxWidth:"640px", margin:"0 auto" }}>
          ...
          <a href="https://wa.me/972523930681?text=%D7%A9%D7%9C%D7%95%D7%9D%20%D7%9E%D7%90%D7%99%D7%94%2C%20%D7%90%D7%A9%D7%9E%D7%97%20%D7%9C%D7%A9%D7%9E%D7%95%D7%A2%20%D7%A2%D7%9C%20%D7%99%D7%99%D7%A2%D7%95%D7%A5%20%D7%A8%D7%90%D7%A9%D7%95%D7%A0%D7%99" target="_blank" rel="noopener noreferrer" style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"#F9F7F2", color:"var(--terra-dark)", padding:"15px 40px", borderRadius:"var(--radius-pill)", fontFamily:"var(--font-hebrew)", fontWeight:700, fontSize:"16px", textDecoration:"none", boxShadow:"0 8px 32px rgba(0,0,0,0.25)", transition:"all 250ms ease" }}
            ...
            <SiteIcon name="phone" size={16} /> לשיחת ייעוץ ראשונה בוואטסאפ
          </a>
        </div>
      </section>
    </>
